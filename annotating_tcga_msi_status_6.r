...
#sampleinfo6<-loc1
#save(sampleinfo6,file='/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/batch6_sampleinfo_040115.robj')
#sampleinfo6<-sampleinfo6[match(sampledat6$sample_name,sampleinfo6$SAMPLE_NAME),]
#save(sampleinfo6,file='/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/batch6_sampleinfo_042215.robj')
...
#colnames(sampledat6)<-c('sample_name','peak_avg','peak_sd','num_unstable_ks','num_unstable_raw','msi_status_rf','msi_status_tree','msi_status_rf_scaled','msi_status_tree_scaled','tumor_type')
...
library(ggplot2)
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/frac_unstable_per_cancer_041315.pdf',width=7*2,height=7)
ggplot(sampledat,aes(y=log10(sampledat$num_unstable_raw+0.5),x=sampledat$tumor_type)) + geom_boxplot() + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + geom_hline(aes(yintercept=3.5),linetype='dashed',color='red')
dev.off()

pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/peak_avg_per_cancer_041315.pdf',width=7*2,height=7)
ggplot(sampledat,aes(y=sampledat$peak_avg,x=sampledat$tumor_type)) + geom_boxplot() + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + geom_hline(aes(yintercept=0.0059),linetype='dashed',color='red') + coord_cartesian(ylim = c(-0.01,0.02))
dev.off()

pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/peak_avg_per_cancer_scaled_041315.pdf',width=7*2,height=7)
ggplot(sampledat,aes(y=ave(sampledat$peak_avg,sampledat$tumor_type,FUN=function(x)x-median(x,na.rm=T)),x=sampledat$tumor_type)) + geom_boxplot() + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + geom_hline(aes(yintercept=0.0053),linetype='dashed',color='red') + coord_cartesian(ylim = c(-0.01,0.02))
dev.off()
...
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_prop_rf_041315.pdf',width=7*2,height=7)
...
bardat<-data.frame(cancer=names(msifracs)[-c(1,10)],prop=msifracs[-c(1,10)]*100)
bardat$cancer<-factor(bardat$cancer,levels=bardat$cancer[order(bardat$prop,decreasing=TRUE)])
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_prop_rf_scaled_041315.pdf',width=7*2,height=7)
ggplot(data=bardat, aes(x=cancer, y=prop)) + geom_bar(aes(fill=cancer), stat="identity") + guides(fill=FALSE) + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + ylab('Proportion of MSI individuals (%)') + coord_cartesian(ylim = c(0,50)) + xlab('')
dev.off()
...
bardat<-data.frame(cancer=names(msifracs)[-c(1,10)],prop=msifracs[-c(1,10)]*100)
bardat$cancer<-factor(bardat$cancer,levels=bardat$cancer[order(bardat$prop,decreasing=TRUE)])
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_prop_tree_041315.pdf',width=7*2,height=7)
ggplot(data=bardat, aes(x=cancer, y=prop)) + geom_bar(aes(fill=cancer), stat="identity") + guides(fill=FALSE) + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + ylab('Proportion of MSI individuals (%)') + coord_cartesian(ylim = c(0,50)) + xlab('')
dev.off()
...
bardat<-data.frame(cancer=names(msifracs)[-c(1,10)],prop=msifracs[-c(1,10)]*100)
bardat$cancer<-factor(bardat$cancer,levels=bardat$cancer[order(bardat$prop,decreasing=TRUE)])
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_prop_tree_scaled_041315.pdf',width=7*2,height=7)
ggplot(data=bardat, aes(x=cancer, y=prop)) + geom_bar(aes(fill=cancer), stat="identity") + guides(fill=FALSE) + scale_color_brewer(palette="Set1") + theme_bw(base_size = 20) + ylab('Proportion of MSI individuals (%)') + coord_cartesian(ylim = c(0,50)) + xlab('')
dev.off()
