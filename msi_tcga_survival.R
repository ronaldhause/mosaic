...
linear<-lm(clindat$num_unstable~clindat$yearstobirth+clindat$gender+clindat$tumor_type+clindat$radiationtherapy+clindat$pathologicstage)
summary(linear)

require(party)
require(survival)

#full model for MSI status
full_msi_model<-coxph(formula=Surv(clindat$os,clindat$vitalstatus)~clindat$msi_status+clindat$yearstobirth+clindat$gender+strata(clindat$tumor_type)+strata(clindat$radiationtherapy)+strata(clindat$pathologicstage)) #P = 0.43
full_msi_model_zph<-cox.zph(full_msi_model)

#simple models for MSI status
coxph(formula=Surv(clindat$os,clindat$vitalstatus)~clindat$msi_status+clindat$tumor_type) #P = 0.01
coxph(formula=Surv(clindat$os,clindat$vitalstatus)~clindat$msi_status+strata(clindat$tumor_type)) #P = 0.007

#MSS only
coxph(formula=Surv(clindat2$os,clindat2$vitalstatus)~clindat2$num_unstable+clindat2$tumor_type) #P = 0.31

#full model for num_unstable
fullmod<-coxph(formula=Surv(clindat$os,clindat$vitalstatus)~clindat$num_unstable+clindat$yearstobirth+clindat$gender+strata(clindat$tumor_type)+strata(clindat$radiationtherapy)+strata(clindat$pathologicstage)) #P = 0.30
...
#simple models
coxph(formula=Surv(msicancers$os,msicancers$vitalstatus)~msicancers$num_unstable+strata(msicancers$tumor_type)) #P = 0.0049
coxph(formula=Surv(msicancers$os,msicancers$vitalstatus)~msicancers$msi_status+strata(msicancers$tumor_type)) #P = 0.0099

#num_unstable
b<-coxph(formula=Surv(msicancers$os,msicancers$vitalstatus)~msicancers$num_unstable+msicancers$yearstobirth+msicancers$gender+msicancers$tumor_type+msicancers$radiationtherapy+msicancers$pathologicstage) #P = 0.024

#MSS only
b_mss<-coxph(formula=Surv(msicancers2$os,msicancers2$vitalstatus)~msicancers2$num_unstable+msicancers2$tumor_type+msicancers2$yearstobirth+msicancers2$gender+msicancers2$radiationtherapy+msicancers2$pathologicstage) #P = 0.004

exp(b_mss$coef[1]*100)

#msi status
b<-coxph(formula=Surv(msicancers$os,msicancers$vitalstatus)~msicancers$msi_status+msicancers$yearstobirth+msicancers$gender+msicancers$tumor_type+msicancers$radiationtherapy+msicancers$pathologicstage) #P = 0.23
...
#Figure 6B
b<-coxph(formula=Surv(msicancers$os,msicancers$vitalstatus)~msicancers$num_unstable+msicancers$yearstobirth+msicancers$gender+msicancers$tumor_type+msicancers$radiationtherapy+msicancers$pathologicstage) #P = 0.02

bplot<-coxph(formula=Surv(msicancers$os/365,msicancers$vitalstatus)~strata(msicancers$s_quartile)+msicancers$yearstobirth+msicancers$gender+msicancers$tumor_type+msicancers$radiationtherapy+msicancers$pathologicstage)
...
plotdat2$half<-factor(plotdat2$half,levels=c("high","low"))
...
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_survival_continuous_all_cancers_split_by_cancer_1_072116.pdf',width=12,height=12)
...
pdf('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/msi_survival_continuous_all_cancers_split_by_cancer_2_072116.pdf',width=12,height=12)
par(mfrow=c(3,3),mar=c(4,4,2,1))
for (i in 10:18){
subset<-plotdat[which(plotdat$tumor_type==cancers[i]),]
mypamr.plotsurvival(subset[,8], subset[,3]/365, subset[,2], cols[1:4], lty=c(2,1), lwd=c(2,2),main=cancers[i])
