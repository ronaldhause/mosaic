#load('/net/shendure/vol7/stevesal/MSI_TCGA_project/robjs/TCGA_subset_with_MSI_merged_full_analysis.robj') #1 min
#system.time(load('/net/shendure/vol7/stevesal/MSI_TCGA_project/robjs/TCGA_subset_with_MSI_merged_full_analysis_msi_values_converted_to_numeric.robj')) #3 min
...
library(data.table)
#OR
#system.time(load('/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/TCGA_subset_with_MSI_merged_full_analysis_msi_values_converted_to_numeric.robj')) #3 min
...
#data<-rbindlist(list(ucec2,read,coad))
#system.time(save(data,file='/net/shendure/vol7/MSI_TCGA_project/robjs/TCGA_subset_with_MSI_merged_full_analysis_duplicates_eliminated.robj')) #14 min
...
#data<-droplevels.data.table(data) #takes a long time
...
# a<-match(unique(data$SAMPLE_NAME),data$SAMPLE_NAME)
# peak_diff<-as.numeric(as.matrix(data$PEAK_DIFFERENCE_VALUE))
# ks_vals<-as.numeric(as.matrix(data$KS_VALUE))
# datasmall<-data[a,colnames(data)[6:18]]
# peak_avg<-tapply(as.numeric(as.matrix(ucec$PEAK_DIFFERENCE_VALUE)),ucec$SAMPLE_NAME,function(x)mean(x,na.rm=T))
# peak_sd<-tapply(as.numeric(as.matrix(ucec$PEAK_DIFFERENCE_VALUE)),ucec$SAMPLE_NAME,function(x)sd(x,na.rm=T))
# frac_unstable<-tapply(ks_vals,ucec$SAMPLE_NAME,function(x)length(which(x<0.05))/516876)
# ucecsmall$peak_avg<-peak_avg
# ucecsmall$peak_sd<-peak_sd
# ucecsmall$frac_unstable<-frac_unstable
# ucecsmall2<-ucecsmall[-which(ucecsmall$MSI_STATUS=='NaN'),-c(1:2)]

#data$PEAK_DIFFERENCE_VALUE<-as.numeric(as.matrix(data$PEAK_DIFFERENCE_VALUE))
#data$KS_VALUE<-as.numeric(as.matrix(data$KS_VALUE))
#system.time(save(data,file='/net/shendure/vol7/stevesal/MSI_TCGA_project/robjs/TCGA_subset_with_MSI_merged_full_analysis_duplicates_eliminated_msi_values_converted_to_numeric_nans_corrected.robj')) #14 min
...
# ucecsmall<-data.frame(peak_avg$V1,peak_sd$V1,frac_unstable$V1,MSI=loc1$MSI_STATUS[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)])

# wilcox.test(loc1$Peak_diff[which(test$MSI_STATUS=='MSI-H')],test$Peak_diff[which(test$MSI_STATUS=='MSS')])
# system.time(peak_avg<-data[,mean(PEAK_DIFFERENCE_VALUE,na.rm=T),by=LOCUS_COORDINATES]) #20s

#setkey(data,LOCUS_COORDINATES)
# data2<-data
# setkey(data2,MSI_STATUS)
# mss_dat<-data2["MSS"]
# msp_dat<-data2["MSI-H"]
# data3<-rbind(mss_dat,msp_dat)
# setkey(data2,c('LOCUS_COORDINATES','MSI_STATUS'))
# locs<-unique(data$LOCUS_COORDINATES) #516876
#loc1<-data["10:100008321-100008335"]
# pvals<-c()
# system.time(for (i in 1:100){
#   print(i)
#   loc1<-data[as.character(locs[i])]
# if (length(which(is.na(loc1$PEAK_DIFFERENCE_VALUE)==T))<223){
#   pvals[i]<-wilcox.test(loc1$PEAK_DIFFERENCE_VALUE[which(loc1$MSI_STATUS=='MSI-H')],loc1$PEAK_DIFFERENCE_VALUE[which(loc1$MSI_STATUS=='MSS')])$p.value}
#   else{
#   pvals[i]<-NA
#   }
# })
...
#sampleinfo1<-loc1
#save(sampleinfo1,file='/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/batch1_sampleinfo_030115.robj')
...
#system.time(locus_diffs<-read[,mean(PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSI-H')],na.rm=T)-mean(PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSS')],na.rm=T),by=LOCUS_COORDINATES]) #97s
...
system.time(msi_sig_locs<-msidata[,length(which(KS_VALUE<0.05)),by=LOCUS_COORDINATES]) #4s
system.time(mss_sig_locs<-mssdata[,length(which(KS_VALUE<0.05)),by=LOCUS_COORDINATES]) #10s
...
#90 MSI-H, 287 MSS
#defining KS_VALUE < 0.05, best site is MSI in 11/90 MSI-H samples vs. 10/287 MSS samples

fisher.test(matrix(c(82,90-82,64,287-64),nrow=2,byrow=T))
...
#subset<-data[sublocs]
#setkey(subset,LOCUS_COORDINATES)
...
length(intersect(which(fisher_pvals$qvals<1e-20),which(fisher_odds$V1>1))) #21

#positive numbers = msi
length(which(abs(msi_mean_locus_peaks$V1)>0)) #60540/516876, 11.7%
length(which(abs(mss_mean_locus_peaks$V1)>0)) #67361/516876, 13.0%
length(which(msi_mean_locus_peaks$V1>0)) #54010/516876, 10.4%
length(which(mss_mean_locus_peaks$V1>0)) #36103/516876, 7.0%

#system.time(locus_diffs<-data[,mean(PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSI-H')],na.rm=T)-mean(PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSS')],na.rm=T),by=LOCUS_COORDINATES]) #166s

#system.time(locus_sig_diffs<-data[,wilcox.test(PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSI-H')],PEAK_DIFFERENCE_VALUE[which(MSI_STATUS=='MSS')])$p.value,by=LOCUS_COORDINATES]) #20s
...
2sigloc10<-data[as.character(sig_sites$locus[1:10000])]
...
msi_mss_locus_output$V6<-80-msi_mss_locus_output$V6
msi_mss_locus_output$V7<-270-msi_mss_locus_output$V7
...
write.table(msi_mss_locus_output,'/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/tcga_subset_with_msi_calls_loci_results_020115.txt',quote=F,row.names=F)
write.csv(msi_mss_locus_output,'/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/tcga_subset_with_msi_calls_loci_results_020115.csv',quote=F,row.names=F)

#sampleinfo
setkey(ucec,SAMPLE_NAME)
locusinfo<-ucec["TCGA-A5-A0G1"]
save(locusinfo,file='/net/shendure/vol3/data/nobackup/hauser/Shendure/msi/tcga_locusinfo.robj')
...
#ucecsmall<-data.frame(sample_name=peak_avg$SAMPLE_NAME,peak_avg=peak_avg$V1,peak_sd=peak_sd$V1,frac_unstable=frac_unstable$V1,msi=loc1$MSI_STATUS[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],sigloc1=sigloc1$PEAK_DIFFERENCE_VALUE[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)])
...
# test<-data.frame(ucecsmall,exo1=sigloc1$EXO1[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],mlh1=sigloc1$MLH1[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],mlh3=sigloc1$MLH3[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],msh2=sigloc1$MSH2[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],msh3=sigloc1$MSH3[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],msh6=sigloc1$MSH6[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],pms1=sigloc1$PMS1[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],pms2=sigloc1$PMS2[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],pold1=sigloc1$POLD1[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)],pole=sigloc1$POLE[match(peak_avg$SAMPLE_NAME,loc1$SAMPLE_NAME)])
# test2<-test[which(test$msi=='MSI-H' | test$msi=='MSS'),]
# test2$msi<-factor(test2$msi)

#adding loci with significant enrichment to msi in MSI-H vs. MSS to improve classification accuracy
